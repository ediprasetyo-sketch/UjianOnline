...
$return='essay_grading.php?attempt_id='.$attemptId.'&saved=1';
...
body{background:#f4f7fb}.eg-wrap{max-width:1180px;margin:0 auto;padding:28px}.eg-head{display:flex;justify-content:space-between;gap:20px;align-items:flex-start;margin-bottom:18px}.eg-head h1{margin:0 0 6px;font-size:26px}.eg-sub{color:#60708d}.eg-actions{display:flex;gap:8px}.eg-btn{display:inline-flex;align-items:center;justify-content:center;border:1px solid #bfd0ea;background:#fff;color:#2457ad;border-radius:9px;padding:9px 14px;text-decoration:none;font-weight:700}.eg-btn.primary{background:#2861c4;color:#fff;border-color:#2861c4}.eg-summary{background:#fff;border:1px solid #dce5f2;border-radius:14px;padding:18px;margin-bottom:18px;box-shadow:0 3px 12px rgba(30,60,100,.05)}.eg-progress{height:9px;background:#e8eef7;border-radius:99px;overflow:hidden;margin-top:10px}.eg-progress>span{display:block;height:100%;background:#2e67c7}.eg-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px}.eg-card{background:#fff;border:1px solid #dce5f2;border-radius:14px;padding:18px;box-shadow:0 3px 12px rgba(30,60,100,.05)}.eg-card.disabled{opacity:.7}.eg-qhead{display:flex;justify-content:space-between;gap:10px;align-items:center}.eg-number{font-weight:800;color:#244b86}.eg-badge{font-size:12px;padding:5px 9px;border-radius:999px;background:#fff0d9;color:#9a5500}.eg-badge.done{background:#e7f8ee;color:#137441}.eg-question{margin:15px 0;font-size:16px;line-height:1.55}.eg-img{display:block;max-width:100%;max-height:300px;border-radius:9px;border:1px solid #dce5f2;margin-bottom:12px;object-fit:contain}.eg-panel{border:1px solid #e0e7f1;border-radius:9px;padding:12px;margin-top:10px;background:#f8fafc}.eg-panel.key{background:#fff7e8;border-color:#f2d49c}.eg-label{font-size:12px;font-weight:800;margin-bottom:5px}.eg-answer{white-space:pre-wrap;line-height:1.5}.eg-form{margin-top:14px}.eg-score-row{display:flex;gap:8px;align-items:center}.eg-score{flex:1;min-width:0;padding:10px;border:1px solid #cbd7e8;border-radius:8px;font-size:15px}.quick{display:flex;gap:5px;flex-wrap:wrap;margin:8px 0}.quick button{border:1px solid #cbd7e8;background:#fff;border-radius:7px;padding:5px 8px;font-size:12px;cursor:pointer}.eg-save{border:0;background:#2861c4;color:#fff;border-radius:8px;padding:10px 14px;font-weight:800;cursor:pointer}.eg-note{margin-top:12px;color:#64748b;font-size:13px}.flash{padding:12px 14px;border-radius:9px;background:#e8f8ef;color:#17683d;border:1px solid #b9e7ca;margin-bottom:16px}@media(max-width:800px){.eg-wrap{padding:16px}.eg-head{display:block}.eg-actions{margin-top:12px}.eg-grid{grid-template-columns:1fr}}
</style></head><body><main class="eg-wrap">
<div class="eg-head"><div><a class="eg-btn" href="results.php?id=<?=h($detail['exam_id'])?>&attempt_id=<?=h($attemptId)?>">← Kembali ke Hasil</a><h1>Penilaian Essay</h1><div class="eg-sub"><b><?=h($detail['full_name'])?></b> · <?=h($detail['title'])?> · Attempt #<?=h($attemptId)?></div></div><div class="eg-actions"><a class="eg-btn primary" href="#q-<?=h($enabled[array_key_first($enabled)]['id']??'')?>">Mulai/lanjut menilai</a></div></div>
<?php if(isset($_GET['saved'])):?><div class="flash">Nilai berhasil disimpan dan total attempt diperbarui.</div><?php endif;?>
<section class="eg-summary"><div><b>Progress penilaian</b> — <?=$graded?> / <?=count($enabled)?> soal ber-kunci sudah dinilai · Nilai Essay <?=$gradedTotal?> / <?=$maxTotal?></div><div class="eg-progress"><span style="width:<?=count($enabled)?round($graded/count($enabled)*100):0?>%"></span></div><div class="eg-note">Soal Essay tanpa kunci & poin tidak masuk proses penilaian V1.3.</div></section>
<div class="eg-grid">
<?php foreach($questions as $i=>$a): $active=(int)$a['use_answer_key']===1 && (float)$a['points']>0; $done=$a['essay_score']!==null; ?>
<article class="eg-card <?=!$active?'disabled':''?>" id="q-<?=h($a['id'])?>"><div class="eg-qhead"><div class="eg-number">Soal <?=($i+1)?> · <?=h($a['points'])?> poin</div><span class="eg-badge <?=$done?'done':''?>"><?=!$active?'Tidak dinilai':($done?'Sudah dinilai':'Belum dinilai')?></span></div>
...
<?php endforeach;?>
</div></main><script>document.querySelectorAll('.quick').forEach(q=>{const input=q.parentElement.querySelector('.eg-score');const max=parseFloat(input.max||'0');q.querySelectorAll('button').forEach(b=>b.addEventListener('click',()=>{input.value=(max*parseFloat(b.dataset.score)/100).toFixed(2).replace(/\.00$/,'');input.focus()}));});</script></body></html>
