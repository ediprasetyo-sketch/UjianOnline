...
$progressPercent=count($enabled)>0?($graded/count($enabled)*100):0;
...
<section class="eg-summary"><div class="eg-summary-line"><div><b>Progress penilaian</b> — <?=$graded?> / <?=count($enabled)?> soal berkunci sudah dinilai</div><div class="eg-stat">Selesai <strong><?=round($progressPercent)?>%</strong> · Belum dinilai <strong><?=$remaining?></strong></div></div><div class="eg-progress"><span style="width:<?=round($progressPercent,2)?>%"></span></div><div class="eg-stat" style="margin-top:8px">Nilai Essay <strong><?=h(number_format($gradedTotal,2,'.',''))?> / <?=h(number_format($maxTotal,2,'.',''))?></strong> · <?=h(number_format($percent,2,'.',''))?>% dari poin maksimal</div><?php if($remaining===0 && count($enabled)>0):?><div class="complete"><b>Penilaian Essay selesai.</b> Semua soal yang memiliki kunci jawaban dan poin sudah dinilai.</div><?php endif;?><div class="eg-note">Soal Essay tanpa kunci &amp; poin tidak masuk proses penilaian.</div><div class="eg-nav"><?php foreach($enabled as $n=>$nav): $navDone=$nav['essay_score']!==null; ?><a class="<?=$navDone?'done':'todo'?> <?=$focusId===(int)$nav['id']?'current':''?>" href="#q-<?=h($nav['id'])?>">Soal <?=($n+1)?> <?=$navDone?'✓':'•'?></a><?php endforeach;?></div><div class="eg-tools"><input id="egSearch" class="eg-search" type="search" placeholder="Cari nomor atau isi soal..."><div class="eg-filter" role="group" aria-label="Filter penilaian"><button type="button" class="active" data-filter="all">Semua <span id="countAll">0</span></button><button type="button" data-filter="todo">Belum dinilai <span id="countTodo">0</span></button><button type="button" data-filter="done">Sudah dinilai <span id="countDone">0</span></button></div><span class="eg-filter-count" id="filterCount"></span></div><div id="egDirty" class="eg-dirty">Ada nilai yang sedang diubah dan belum disimpan.</div></section>
...
<article class="eg-card <?=$focused?'focused ':''?><?=!$active?'disabled':''?>" id="q-<?=h($a['id'])?>" data-status="<?=!$active?'disabled':($done?'done':'todo')?>" data-search="<?=h('soal '.($i+1).' '.$a['question_text'])?>"><div class="eg-qhead"><div class="eg-number">Soal <?=($i+1)?> · <?=h($a['points'])?> poin</div><span class="eg-badge <?=$done?'done':''?>"><?=!$active?'Tidak dinilai':($done?'Sudah dinilai':'Belum dinilai')?></span></div>
<div class="eg-question"><?=nl2br(h($a['question_text']))?></div>
<?php if(!empty($a['question_image'])):?><img class="eg-img" src="<?=h(question_image_url($a['question_image']))?>" alt="Gambar soal" loading="lazy"><?php endif;?>
...
<?php endforeach;?>
</div><div id="egNoResults" class="eg-no-results">Tidak ada soal yang cocok dengan filter atau pencarian.</div>
</main><script>
const cards=[...document.querySelectorAll('.eg-card')];
const search=document.getElementById('egSearch');
const filterButtons=[...document.querySelectorAll('.eg-filter button')];
const countAll=document.getElementById('countAll');
const countTodo=document.getElementById('countTodo');
const countDone=document.getElementById('countDone');
const filterCount=document.getElementById('filterCount');
const noResults=document.getElementById('egNoResults');
let activeFilter='all';
function applyFilter(){
 const term=(search.value||'').trim().toLowerCase();
 let visible=0, all=0,todo=0,done=0;
 cards.forEach(card=>{
   const status=card.dataset.status;
   if(status==='disabled') return;
   all++; if(status==='todo') todo++; if(status==='done') done++;
 ...
 countAll.textContent=all; countTodo.textContent=todo; countDone.textContent=done;
 filterCount.textContent=visible+' soal tampil';
 noResults.style.display=visible?'none':'block';
}
filterButtons.forEach(btn=>btn.addEventListener('click',()=>{activeFilter=btn.dataset.filter;filterButtons.forEach(x=>x.classList.toggle('active',x===btn));applyFilter();}));
search.addEventListener('input',applyFilter);
applyFilter();

document.querySelectorAll('.quick').forEach(q=>{const input=q.parentElement.querySelector('.eg-score');const max=parseFloat(input.max||'0');q.querySelectorAll('button').forEach(b=>b.addEventListener('click',()=>{input.value=(max*parseFloat(b.dataset.score)/100).toFixed(2).replace(/\\.00$/,'');input.focus();q.querySelectorAll('button').forEach(x=>x.classList.toggle('active',x===b));input.dispatchEvent(new Event('input',{bubbles:true}));}));});

const dirtyBox=document.getElementById('egDirty');
let dirty=false;
function markDirty(){dirty=true;dirtyBox.classList.add('show');}
document.querySelectorAll('.eg-score').forEach(input=>input.addEventListener('input',markDirty));
document.querySelectorAll('.eg-form').forEach(form=>form.addEventListener('submit',()=>{dirty=false;dirtyBox.classList.remove('show');}));
window.addEventListener('beforeunload',e=>{if(!dirty)return;e.preventDefault();e.returnValue='';});
document.querySelectorAll('a').forEach(link=>link.addEventListener('click',e=>{if(!dirty)return;if(!confirm('Ada nilai yang belum disimpan. Tetap meninggalkan halaman?'))e.preventDefault();}));
