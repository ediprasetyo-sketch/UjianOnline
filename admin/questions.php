...
?>
<!doctype html><html lang="id"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Kelola Soal</title><link rel="stylesheet" href="assets/admin-ui.css"><link rel="stylesheet" href="assets/ui-consistency-v6380.css?v=6.3.80"></head><body>
...
<div id="gradingFields"><label style="display:flex;gap:8px;align-items:center"><input type="checkbox" name="use_answer_key" id="useAnswerKey" checked> Gunakan Kunci Jawaban &amp; Poin</label><div id="gradingDetails"><div id="standard-answer-key"><label>Kunci jawaban</label><select name="correct_option" id="correctOption"></select></div><div id="essay-answer-key" style="display:none"><label>Jawaban acuan <span class="ui-sub">(opsional)</span></label><textarea name="essay_answer_key" placeholder="Kunci atau jawaban yang diharapkan"></textarea></div><div id="points-field"><label>Poin</label><input type="number" name="points" min="0" step="0.5" value="1"></div></div></div>
...
function sync(){
  ...
  document.getElementById('essay-answer-key').style.display=isEssay&&useKey.checked?'block':'none';
  document.getElementById('points-field').style.display=useKey.checked?'block':'none';
